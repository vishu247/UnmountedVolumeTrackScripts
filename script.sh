...
az login --service-principal --username $USERNAME --password $PASSWORD --tenant $TENANT_ID

# Azure CLI command to list all disks
disks=$(az disk list --query "[?diskState=='Unattached' && tags.required != 'true']" )

# Prompt user to choose whether to delete unattached disks
UNMOUNTED_VOLUME_THRESHOLD=$THRESHOLD

# Discord webhook url
YOUR_DISCORD_WEBHOOK_URL=$URL
